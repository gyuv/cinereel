 ...
 * TMDb fetch layer
 * ---------------------------------------------------------------------------
 * Get a free API key at https://www.thoviedb.org/settings/api and set:
...
// TMDB doesn't have a separate "Anime" endpoint, but we can filter TV shows by Genre 16 (Animation)
// and often 10765 (Sci-Fi & Fantasy) to get accurate Anime results.
const ANIME_GENRES = [16, 10765]; 

export async function searchAnime(filters: SearchFilters): Promise<SearchResponse> {
 if (!hasKey) return mockAnimeSearch(filters);

 // Search Multi (for text queries)
 ...
 // Check if it has anime-relevant genres
 if (item.genre_ids?.some((gid: number) => ANIME_GENRES.includes(gid))) {
 results.push(mapSummary(item, "anime"));
 }
 }
 }
 ...
 // Filter-only discovery (Discover TV with Genre 16)
 const genreParam = filters.genres.length ? filters.genres.join(",") : undefined;
 const tvRes = await tmdbFetch<any>("/discover/tv", {
 page: String(filters.page),
 with_genres: genreParam || "16", // Default to Animation if no genre specified
 ...
 const results = tvRes.results.map((r: any) => {
 // Only include if it matches anime genres or if we are just browsing popular
 if (filters.query.trim() === "" || r.genre_ids?.some((gid: number) => ANIME_GENRES.includes(gid))) {
 return mapSummary(r, "anime");
 }
 return null;
...
function mockAnimeSearch(filters: SearchFilters): SearchResponse {
 // Mock Anime Data
 const mockTitles = [
 { id: 1, title: "Cyber Edge", year: "2024", genreIds: [16, 10765] },
 { id: 2, title: "Spirit Woods", year: "2023", genreIds: [16, 10752] },
 { id: 3, title: "Academy of Magic", year: "2022", genreIds: [16, 35] },
 ];
 
 let results = mockTitles.filter(m => filters.query.trim() ? m.title.toLowerCase().includes(filters.query.toLowerCase()) : true)
 .map((m, i) => ({
 id: m.id + (i * 100),
 kind: "anime" as const,
 title: m.title,
 ...
 posterUrl: null,
 rating: 8 + (i % 2),
 overview: "Mock anime data until API key is set.",
 genreIds: m.genreIds,
 }));
...
export async function searchMedia(filters: SearchFilters): Promise<SearchResponse> {
 ...
 // Query TMDb's multi-search when there's free text
 if (isPersonQuery) {
 const raw = await tmdbFetch<any>("/search/multi", {
 query: filters.query,
 page: String(filters.page),
...
export async function getMediaDetail(kind: MediaKind, id: number): Promise<MediaDetail> {
 ...
 // If kind is 'anime', we fetch from /tv/id because that's how TMDB stores anime
 const apiKind = kind === "anime" ? "tv" : kind;

 const [detail, credits, videos, providers] = await Promise.all([
 tmdbFetch<any>(`/${apiKind}/${id}`),
 tmdbFetch<any>(`/${apiKind}/${id}/credits`),
 tmdbFetch<any>(`/${apiKind}/${id}/videos`),
 tmdbFetch<any>(`/${apiKind}/${id}/watch/providers`),
 ]);
 ...
 return {
 ...mapSummary(detail, kind), // Pass 'kind' (anime) to mapSummary so it sets kind:'anime'
